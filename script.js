...
let typewriterTimers = []; // For typewriter effect
let lastGlitchTime = 0;    // For glitch effect timing
let glitchOffsets = [];    // Store random offsets for glitch
...
// Apply the Sobel operator to a 2D image.
...
function generateASCII(img) {
  ...
  const animateEnabled = document.getElementById('animate').checked;
  const animType = document.getElementById('animationType').value;
  const asciiArtElem = document.getElementById('ascii-art');
  
  if (animateEnabled) {
    // Wrap each ASCII art line in its own container.
    const lines = ascii.split("\n");
    let html = "";
    lines.forEach((line, index) => {
      const escapedLine = line.replace(/"/g, '&quot;');
      html += `<div class="ascii-line" data-full-text="${escapedLine}">${(animType === 'typewriter' ? "" : line)}</div>`;
    });
    asciiArtElem.innerHTML = html;
    
    if (animType === 'wave' || animType === 'bounce' || animType === 'colorCycle' || animType === 'glitch') {
      startAnimation();
    ...
    asciiArtElem.textContent = ascii;
  }
}
...
function generateContourASCII(img) {
  ...
  // ----- Animation Integration (same as above) -----
  const animateEnabled = document.getElementById('animate').checked;
  const animType = document.getElementById('animationType').value;
  const asciiArtElem = document.getElementById('ascii-art');
...
// General animation loop for wave, bounce, colorCycle and glitch effects.
function animateFrame(timestamp) {
  const speed = parseFloat(document.getElementById('animationSpeed').value);
  const baseAmplitude = parseFloat(document.getElementById('animationAmplitude').value);
  const oscAmplitude = parseFloat(document.getElementById('oscillatorAmplitude').value);
  const oscFrequency = parseFloat(document.getElementById('oscillatorFrequency').value);
  const animType = document.getElementById('animationType').value;
  const hueBase = parseFloat(document.getElementById('animationHue').value);
  const colorMode = document.getElementById('colorMode').value;
  const lines = document.querySelectorAll('.ascii-line');
  
  // Combine base amplitude with oscillator.
  const effectiveAmplitude = baseAmplitude + oscAmplitude * Math.sin(timestamp / 1000 * oscFrequency);
  
  // Function to choose a color based on the current mode.
  function getColor(index) {
    if (colorMode === 'rainbow') {
      const hue = (hueBase + (timestamp / 10 * speed) + index * 10) % 360;
      ...
      const gradientStr = document.getElementById('gradientColors').value;
      const colors = gradientStr.split(',').map(s => s.trim()).filter(s => s.length);
      if (colors.length > 0) {
        const colorIndex = Math.floor((timestamp / 1000 * speed + index) % colors.length);
  ...
  if (animType === 'wave') {
    lines.forEach((lineElem, index) => {
      const offsetX = effectiveAmplitude * Math.sin((timestamp / 1000 * speed) + index);
      lineElem.style.transform = `translateX(${offsetX}px)`;
      lineElem.style.color = getColor(index);
    });
  } else if (animType === 'bounce') {
    lines.forEach((lineElem, index) => {
      const offsetY = effectiveAmplitude * Math.abs(Math.sin((timestamp / 1000 * speed) + index));
      lineElem.style.transform = `translateY(${offsetY}px)`;
      lineElem.style.color = getColor(index);
    });
  } else if (animType === 'colorCycle') {
    lines.forEach((lineElem, index) => {
      const offsetX = effectiveAmplitude * Math.sin((timestamp / 1000 * speed) + index);
      lineElem.style.transform = `translateX(${offsetX}px)`;
      lineElem.style.color = getColor(index);
    ...
    const glitchInterval = 200; // milliseconds
    if (timestamp - lastGlitchTime > glitchInterval) {
      glitchOffsets = [];
      lines.forEach((lineElem, index) => {
        const offsetX = (Math.random() - 0.5) * effectiveAmplitude * 2;
        const offsetY = (Math.random() - 0.5) * effectiveAmplitude * 2;
        glitchOffsets[index] = { offsetX, offsetY, glitchColor: getColor(index) };
      });
      lastGlitchTime = timestamp;
    }
    lines.forEach((lineElem, index) => {
      const glitch = glitchOffsets[index] || { offsetX: 0, offsetY: 0, glitchColor: '' };
      lineElem.style.transform = `translate(${glitch.offsetX}px, ${glitch.offsetY}px)`;
      lineElem.style.color = glitch.glitchColor;
    });
  }
...
// Typewriter effect: gradually reveal each character.
function startTypewriterAnimation() {
  const speed = parseFloat(document.getElementById('animationSpeed').value);
  const lines = document.querySelectorAll('.ascii-line');
  stopTypewriterAnimation();
  lines.forEach((lineElem, index) => {
    const fullText = lineElem.getAttribute('data-full-text');
    lineElem.textContent = "";
    let charIndex = 0;
    const delayBeforeStart = index * 100; // Stagger each line.
    const timer = setTimeout(function animateChars() {
      if (charIndex < fullText.length) {
        lineElem.textContent += fullText.charAt(charIndex);
        charIndex++;
        const charDelay = Math.max(20, 100 - speed * 2);
        setTimeout(animateChars, charDelay);
      }
    }, delayBeforeStart);
...
// ---------------- Transparent GIF (Video) Export ----------------

function downloadTransparentGIF() {
  // Retrieve the ASCII art lines. If using animations, get each line's full text.
  const asciiArtElem = document.getElementById('ascii-art');
  let asciiLines = [];
  const lineElems = asciiArtElem.querySelectorAll('.ascii-line');
  if (lineElems.length > 0) {
    lineElems.forEach(lineElem => {
      asciiLines.push(lineElem.getAttribute('data-full-text'));
    });
  } else {
    // Fallback for plain pre text.
    asciiLines = asciiArtElem.textContent.split("\n");
  }
  asciiLines = asciiLines.filter(line => line.trim().length > 0);
  if (asciiLines.length === 0) {
    alert("No ASCII art to export.");
    return;
  }

  // Read export options from controls.
  const duration = parseFloat(document.getElementById('exportDuration').value) || 3; // seconds
  const fps = parseFloat(document.getElementById('exportFrameRate').value) || 15;
  const frameCount = Math.ceil(duration * fps);
  const frameDelay = 1000 / fps; // delay per frame (ms)

  // Set up an offscreen canvas for GIF frames.
  const scaleFactor = 2;
  const tempCanvas = document.createElement('canvas');
  const tempCtx = tempCanvas.getContext('2d');
  const exportFontSize = baseFontSize * scaleFactor;
  tempCtx.font = `${exportFontSize}px Consolas, Monaco, "Liberation Mono", monospace`;
  let maxLineWidth = 0;
  ...
  const textWidth = Math.ceil(maxLineWidth);
  const textHeight = Math.ceil(asciiLines.length * lineHeight);
  const borderMargin = 20 * scaleFactor;
  const canvasWidth = textWidth + 2 * borderMargin;
  const canvasHeight = textHeight + 2 * borderMargin;
  ...
  // Ensure font and textBaseline are set.
  gifCtx.font = `${exportFontSize}px Consolas, Monaco, "Liberation Mono", monospace`;
  gifCtx.textBaseline = 'top';

  // Create the GIF encoder (using gif.js)
  const gif = new GIF({
    workers: 2,
    quality: 10,
    width: canvasWidth,
  ...
  // Helper: Render one frame of the ASCII art animation onto the canvas.
  function renderGIFFrame(ctx, timestamp) {
    ctx.clearRect(0, 0, canvasWidth, canvasHeight);
    // Reinitialize the context state each frame.
    ctx.font = `${exportFontSize}px Consolas, Monaco, "Liberation Mono", monospace`;
    ...
    function getColor(index) {
      if (colorMode === 'rainbow') {
        ...
        return document.body.classList.contains('light-mode') ? "#000" : "#eee";
      } else if (colorMode === 'custom') {
        return document.getElementById('customColor').value;
      } else if (colorMode === 'gradient') {
        const gradientStr = document.getElementById('gradientColors').value;
    ...
    asciiLines.forEach((line, index) => {
      let x = borderMargin;
      let y = borderMargin + index * lineHeight;
      let offsetX = 0, offsetY = 0;
      if (animType === 'wave') {
        offsetX = effectiveAmplitude * Math.sin((timestamp / 1000 * speed) + index);
      } else if (animType === 'bounce') {
        offsetY = effectiveAmplitude * Math.abs(Math.sin((timestamp / 1000 * speed) + index));
      } else if (animType === 'colorCycle') {
        offsetX = effectiveAmplitude * Math.sin((timestamp / 1000 * speed) + index);
      } else if (animType === 'glitch') {
        offsetX = (Math.random() - 0.5) * effectiveAmplitude * 2;
        offsetY = (Math.random() - 0.5) * effectiveAmplitude * 2;
      }
  ...
  function addFrame() {
    if (currentFrame < frameCount) {
      const simulatedTimestamp = currentFrame * frameDelay;
      renderGIFFrame(gifCtx, simulatedTimestamp);
      gif.addFrame(gifCanvas, { delay: frameDelay });
      currentFrame++;
...
document.getElementById('animationSpeed').addEventListener('input', function() {
  document.getElementById('animationSpeedVal').textContent = this.value;
});
document.getElementById('animationAmplitude').addEventListener('input', function() {
  document.getElementById('animationAmplitudeVal').textContent = this.value;
});
document.getElementById('oscillatorFrequency').addEventListener('input', function() {
  document.getElementById('oscillatorFrequencyVal').textContent = this.value;
});
document.getElementById('oscillatorAmplitude').addEventListener('input', function() {
  document.getElementById('oscillatorAmplitudeVal').textContent = this.value;
});
document.getElementById('animationHue').addEventListener('input', function() {
  document.getElementById('animationHueVal').textContent = this.value;
});
document.getElementById('colorMode').addEventListener('change', function() {
  const customControl = document.getElementById('customColorControl');
  const gradientControl = document.getElementById('gradientColorsControl');
  if (this.value === 'custom') {
    customControl.style.display = 'flex';
    gradientControl.style.display = 'none';
  } else if (this.value === 'gradient') {
    gradientControl.style.display = 'flex';
...
document.getElementById('animationType').addEventListener('change', updateSettings);
document.getElementById('animate').addEventListener('change', updateSettings);
...
document.getElementById('downloadGifBtn').addEventListener('click', downloadTransparentGIF);
...
function resetSettings() {
  ...
  document.getElementById('animationType').value = 'typewriter';
  document.getElementById('animationSpeed').value = 30;
  document.getElementById('animationSpeedVal').textContent = "30";
  document.getElementById('animationAmplitude').value = 10;
  document.getElementById('animationAmplitudeVal').textContent = "10";
  document.getElementById('oscillatorFrequency').value = 1;
  document.getElementById('oscillatorFrequencyVal').textContent = "1";
  document.getElementById('oscillatorAmplitude').value = 0;
  document.getElementById('oscillatorAmplitudeVal').textContent = "0";
  document.getElementById('animationHue').value = 0;
  document.getElementById('animationHueVal').textContent = "0";
  document.getElementById('colorMode').value = 'rainbow';
  document.getElementById('customColorControl').style.display = 'none';
  document.getElementById('gradientColorsControl').style.display = 'none';
